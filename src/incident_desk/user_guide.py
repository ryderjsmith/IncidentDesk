...
def show_user_guide(parent) -> None:
    ...
    h2("Managing Units, Locations, Types, Driver Codes & Ambulances")
    ...
    body("  • Ambulances — ambulance identifiers selectable on the incident form.")
    ...
    body("")
    body("Manage Units and Manage Ambulances also colour each row by availability:")
    body("  • Green — not currently assigned to any active incident")
    body("  • Red — assigned to at least one active (not-cleared) incident")
    body("Units behave differently from Ambulances here: a unit already assigned as Primary on an")
    body("active incident is hidden from the Primary Unit dropdown on new incidents. Ambulances are")
    body("never filtered — they remain selectable regardless of availability, and the colour coding")
    body("in Manage Ambulances is informational only. This lets you assign the same ambulance to")
    body("multiple incidents when circumstances require it.")
    body("")
